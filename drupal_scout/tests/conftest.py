...
def make_composer_project(tmp_path):
    ...
    def _create(root_path=None, packages_map=None, patches_inline=None, patches_file_data=None, patches_file_name="composer.patches.json"):
        ...
        if packages_map:
            for pkg_name, info in packages_map.items():
                if isinstance(info, str):
                    ...
                    install_path = info.get("install_path", f"../../web/modules/contrib/{pkg_name.split('/')[-1]}")
                    version = info.get("version", "1.0.0")
                else:
                    install_path = f"../../web/modules/contrib/{pkg_name.split('/')[-1]}"
                    version = "1.0.0"
        ...
        if patches_file_data is not None:
            ...
            content = {"patches": patches_file_data} if isinstance(patches_file_data, dict) and "patches" not in patches_file_data else patches_file_data
