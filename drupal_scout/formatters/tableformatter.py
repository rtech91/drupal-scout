...
class TableFormatter(Formatter):
    ...
    def format(self, modules: list[Module]) -> Any:
        ...
        if has_git_audit:
            ...
            if clean_count > 0:
                detail_texts.append(f"[dim]✔ {clean_count} module(s) clear with no uncommitted files, commits, or patches.[/dim]")

            if detail_texts:
                audit_panel = Panel("\n\n".join(detail_texts), title="Deep Scan Details", border_style="cyan", box=box.ROUNDED)
