...
class WorkersManager:
    ...
    def __init__(
        ...
        self.concurrency_limit = (
            concurrency_limit if concurrency_limit >= 1 else (cpu_count() or 4)
        )
    ...
    async def run(self):
        ...
        with self.output.progress_bar() as progress:
            main_task = progress.add_task(
                "[cyan]Scanning modules...", total=len(self.modules)
            )
            ...
            async with asyncio.TaskGroup() as tg:
                for module in self.modules:
                    ...
                    tg.create_task(
                        run_worker_with_progress(worker, semaphore, progress, main_task)
                    )
